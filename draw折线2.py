...
performance["任务1"] = np.array([
    [0.45, 0.50, 0.55, 0.60],  # 基准方法1在10%、30%、50%、100%数据量下的表现
    [0.55, 0.60, 0.65, 0.70],  # 基准方法1+Ours的表现
    [0.60, 0.65, 0.70, 0.75],  # 基准方法2的表现
    [0.70, 0.75, 0.80, 0.85],  # 基准方法2+Ours的表现
    [0.75, 0.80, 0.85, 0.90],  # 基准方法3的表现
    [0.85, 0.90, 0.93, 0.96]   # 基准方法3+Ours的表现
])

# 任务2的数据
performance["任务2"] = np.array([
    [0.42, 0.48, 0.53, 0.58],  # 基准方法1的表现
    [0.52, 0.58, 0.63, 0.68],  # 基准方法1+Ours的表现
    [0.58, 0.64, 0.69, 0.74],  # 基准方法2的表现
    [0.68, 0.74, 0.79, 0.84],  # 基准方法2+Ours的表现
    [0.73, 0.79, 0.84, 0.89],  # 基准方法3的表现
    [0.83, 0.89, 0.92, 0.95]   # 基准方法3+Ours的表现
])

# 任务3的数据
performance["任务3"] = np.array([
    [0.44, 0.49, 0.54, 0.59],  # 基准方法1的表现
    [0.54, 0.59, 0.64, 0.69],  # 基准方法1+Ours的表现
    [0.59, 0.64, 0.69, 0.74],  # 基准方法2的表现
    [0.69, 0.74, 0.79, 0.84],  # 基准方法2+Ours的表现
    [0.74, 0.79, 0.84, 0.89],  # 基准方法3的表现
    [0.84, 0.89, 0.92, 0.95]   # 基准方法3+Ours的表现
])

# 任务4的数据
performance["任务4"] = np.array([
    [0.43, 0.49, 0.54, 0.59],  # 基准方法1的表现
    [0.53, 0.59, 0.64, 0.69],  # 基准方法1+Ours的表现
    [0.58, 0.64, 0.69, 0.74],  # 基准方法2的表现
    [0.68, 0.74, 0.79, 0.84],  # 基准方法2+Ours的表现
    [0.73, 0.79, 0.84, 0.89],  # 基准方法3的表现
    [0.83, 0.89, 0.92, 0.95]   # 基准方法3+Ours的表现
...
for ax_idx, task in enumerate(tasks):
    ...
    for i, base in enumerate(base_methods):
        ...
        # ✅ 添加阴影填充区域（两条线之间）
        ax.fill_between(x, arr[idx_base], arr[idx_ours],
